...
import 'notification_unread_card_model.dart';
export 'notification_unread_card_model.dart';

class NotificationUnreadCardWidget extends StatefulWidget {
  const NotificationUnreadCardWidget({
    super.key,
    this.isSelectPage,
  });

  final String? isSelectPage;

  @override
  State<NotificationUnreadCardWidget> createState() =>
      _NotificationUnreadCardWidgetState();
}

class _NotificationUnreadCardWidgetState
    extends State<NotificationUnreadCardWidget> {
  late NotificationUnreadCardModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => NotificationUnreadCardModel());
          ...
          child: Builder(
            builder: (context) {
              if (!FFAppState().inProgress) {
                return Builder(
                  builder: (context) {
                    final list = FFAppState().NotificationUnread.toList();
                    if (list.isEmpty) {
                      return Center(
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, listIndex) {
                        ...
                        return NotificationComponentWidget(
                          key: Key('Keyitt_${listIndex}_of_${list.length}'),
                          name: getJsonField(
                            listItem,
                            r'''$.name''',
                          ).toString(),
                          ...
                          incidentIid: getJsonField(
                              ...
                              : '',
                          action: () async {
                            _model.apiResultl2d = await APIsAllGroup
                            ...
                            if ((_model.apiResultl2d?.succeeded ?? true)) {
                              if (widget.isSelectPage == 'Blacklist') {
                                await Future.wait([
                                  Future(() async {
                                  ...
                                  Future(() async {
                                    ...
                                    FFAppState().unreadCount = APIsAllGroup
                                        .countUnreadNotificationCall
                                        .count(
                                      (_model.blacklistCountUnread?.jsonBody ??
                                          ''),
                                    )!;
                                    FFAppState().update(() {});
                                  }),
                                ]);
                              } else if (widget.isSelectPage == 'Escape') {
                                await Future.wait([
                                  Future(() async {
                                    ...
                                    FFAppState().unreadCount = APIsAllGroup
                                        .countUnreadNotificationCall
                                        .count(
                                      (_model.escapeCountUnread?.jsonBody ??
                                          ''),
                                    )!;
                                    FFAppState().update(() {});
                                  }),
                              ...
                              } else if (widget.isSelectPage == 'Theft') {
                                await Future.wait([
                                  Future(() async {
                                    _model.theftCountUnread = await APIsAllGroup
                                        .countUnreadNotificationCall
                                        .call(
                                      group: 3,
                                      authToken: FFAppState().token,
                                    ...
                                    FFAppState().unreadCount = APIsAllGroup
                                        .countUnreadNotificationCall
                                        .count(
                                      (_model.theftCountUnread?.jsonBody ?? ''),
                                    )!;
                                    FFAppState().update(() {});
                                  }),
                                  Future(() async {
                              ...
                              } else if (widget.isSelectPage == 'Camera') {
                                await Future.wait([
                                  Future(() async {
                                    ...
                                    FFAppState().unreadCount = APIsAllGroup
                                        .countUnreadNotificationCall
                                        .count(
                                      (_model.cameraCountUnread?.jsonBody ??
                                          ''),
                                    )!;
                                    FFAppState().update(() {});
                                  }),
