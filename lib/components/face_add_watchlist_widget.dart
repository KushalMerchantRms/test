...
import 'face_add_watchlist_model.dart';
export 'face_add_watchlist_model.dart';

class FaceAddWatchlistWidget extends StatefulWidget {
  const FaceAddWatchlistWidget({
    super.key,
    required this.photo,
    required this.time,
  ...
  @override
  State<FaceAddWatchlistWidget> createState() => _FaceAddWatchlistWidgetState();
}

class _FaceAddWatchlistWidgetState extends State<FaceAddWatchlistWidget> {
  late FaceAddWatchlistModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => FaceAddWatchlistModel());
            ...
            children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Container(
                      ...
                      child: Padding(
                        padding:
                            EdgeInsetsDirectional.fromSTEB(5.0, 11.0, 0.0, 5.0),
                        child: Column(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Row(
              ...
              RichText(
                ...
                text: TextSpan(
                  children: [
                    TextSpan(
                      text: 'Reason for adding to watchlisting? ',
                      style: FlutterFlowTheme.of(context).bodyMedium.override(
                            fontFamily: 'Inter',
                            color: Color(0xFF464646),
                            fontSize: 12.0,
              ...
              if (_model.comment != null && _model.comment != '')
                Align(
                  ...
                  child: FFButtonWidget(
                    onPressed: () async {
                      _model.apiResultle2 =
                          await OtherAPIsGroup.facesBlacklistCall.call(
                        id: widget.customerId,
                        comments: _model.comment,
                        authToken: FFAppState().token,
                      );

                      if ((_model.apiResultle2?.succeeded ?? true)) {
                        _model.currentdate = await actions.getTimeInTimeZone(
                          FFAppState().userTimeZone,
                        );
                        _model.faceapisuccess =
                          ...
                          fromDate: () {
                            if (widget.fromDate == 'Today') {
                              ...
                              return functions.getRangeOfDateFace(
                                  'Last 1 days', _model.currentdate);
                            } else if (widget.fromDate == 'Last 7 days') {
                              return functions.getRangeOfDateFace(
                                  'Last 6 days', _model.currentdate);
                            } else if (widget.fromDate == 'Last 30 days') {
                              return functions.getRangeOfDateFace(
                                  'Last 29 days', _model.currentdate);
                            } else {
                        ...
                        if ((_model.faceapisuccess?.succeeded ?? true)) {
                          FFAppState().faceList =
                              (_model.faceapisuccess?.jsonBody ?? '')
                                  .toList()
                                  .cast<dynamic>();
                          FFAppState().update(() {});
                          ...
                          await actions.showSnackBar(
                            context,
                            'Added to Watchlist ',
                            'Customer added to watchlist. You’ll be alerted on their next visit.',
                          );
                        }
                      } else {
                        Navigator.pop(context);
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              OtherAPIsGroup.facesBlacklistCall.error(
                                (_model.apiResultle2?.jsonBody ?? ''),
                              )!,
                              style: TextStyle(
                                color: FlutterFlowTheme.of(context)
                                    .secondaryBackground,
